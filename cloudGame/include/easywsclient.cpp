...
class _RealWebSocket : public easywsclient::WebSocket
{
  public:
    ...
    bool useMask;
    bool isRxBad;
    std::shared_ptr<std::thread> m_pingThread;
    std::shared_ptr<std::condition_variable> m_pingThreadEvent;
    std::mutex m_mutex;
    bool m_closeSocket;
    ...
    void poll(int timeout) { // timeout in milliseconds
        if (readyState == CLOSED) {
            if (timeout > 0) {
                ...
                std::cout << "ERROR: poll connection is disconnect1!\n" << std::endl;
            }
            return;
        }
        if (timeout != 0) {
        ...
        while (txbuf.size()) {
            ...
            if (false) { } // ??
                ...
                //MessageBoxA(NULL , std::to_string(ret).c_str() , "32@32" , MB_OK);
    ...
    virtual void _dispatchBinary(BytesCallback_Imp & callable) {
        ...
        while (true) {
            ...
            if (false) { }
                ...
                //MessageBoxA(NULL , "testPing", "34Error34" ,MB_OK);
                if (ws.mask) { for (size_t i = 0; i != ws.N; ++i) { rxbuf[i+ws.header_size] ^= ws.masking_key[i&0x3]; } }
...
//            m_pingThreadEvent.reset();
        ...
        if(readyState == CLOSING || readyState == CLOSED) { return; }
        readyState = CLOSING;
        //MessageBoxA(NULL , "close() close 1!" , "2121" , MB_OK);
